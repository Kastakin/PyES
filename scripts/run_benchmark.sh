...
hyperfine -r 5 --export-json "benchmark/benchmark_results/results.json"\
            --setup "source /home/lorenzo/.cache/pypoetry/virtualenvs/pyes-YsROe161-py3.10/bin/activate"\
            --parameter-list file_name ac,phoenix_var,urine_silvia_modelA_solid_var,urine_silvia_modelA_solid_var_error\
            "python benchmark/test.py mockup/{file_name}.json"\
            --cleanup "poetry run pyinstrument -o benchmark/profiles/profile_{file_name}.html benchmark/test.py mockup/{file_name}.json & (deactivate  || true)"
